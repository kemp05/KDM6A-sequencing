...
$results = "amp_"."$ARGV[0]"."_calls_gte_5.txt";
#chomp($results);
open(OUTPUT, ">$results");

# hash table population
%TABLE=();
open (INPUT3, "amp_"."$ARGV[0]"."_SLX_14583_read_counts_hash.txt") or die "\nAmp file could not be found\n\n";
while (<INPUT3>) {
	chomp ($_);
	@AMP = split('\t',$_);
	$index = 1;
...
# mutation calling: filter all individual sample values with cut-offs etc.
		...
		while ($index <= scalar@NTs){
			if (($NTs[$index] >= 5) && (($NTs[$index] >= $cut_off) || ($NTs[$index] >= $array_mean *4))){
#			if ($NTs[$index] >= 5){
				print OUTPUT "$ARGV[0]\tFLD0$SAMPLES[$index]\t$nt_position\t$base\t$NTs[$index]\t$array_mean\t$array_stdev\t$values\n";				
			}
			$index = $index +1;
		}
	}
