...
while (<INPUT3>) {
	...
	$results = "amp_"."$ARGV[0]"."_SLX_14583_read_counts_hash.txt";
	open(OUTPUT, ">>$results");
	...
	while (<INPUT>) {
		...
		if ($input_file =~ /.fq/){
			...
			($lib,$fld,$hft) = split(/_/,$input_file);
			...
			while (<INPUT2>) {
				...
				if (($_ =~ /^$start_term/) && ($_ =~ /$end_term$/) && (length($_) == $total_length)){
					...
					while ($index <= $end +1){
						...
						$term = "$AMP[$index-7]"."$AMP[$index-6]"."$AMP[$index-5]"."$AMP[$index-4]"."$AMP[$index-3]"."$AMP[$index-2]"."$AMP[$index-1]".
							"([ACGT])"."$AMP[$index+1]"."$AMP[$index+2]"."$AMP[$index+3]"."$AMP[$index+4]"."$AMP[$index+5]"."$AMP[$index+6]"."$AMP[$index+7]";
